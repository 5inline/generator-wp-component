...
Generator.prototype.addScssToSass = function (script)
{
	try {
		...
		angularUtils.rewriteFile({
			...
			splicable: [
				"@import '../../"+ script.toLowerCase().replace(/\\/g, '/') +"';"
			]
		});
	} catch (e) {
		this.log.error(chalk.yellow(
...
Generator.prototype.phpTemplate = function (src, dest)
{
	yeoman.generators.Base.prototype.template.apply(this, [
		src + this.phpSuffix,
		path.join( this.env.options.appPath, dest.toLowerCase()) + this.phpSuffix
	]);
}
...
Generator.prototype.generateSource = function (appTemplate, targetDirectory, skipAdd)
{
	...
	this.phpTemplate( appTemplate, path.join(targetDirectory, this.name, this.name) );
